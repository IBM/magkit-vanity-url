...
public class VanityUrlModuleVersionHandler extends DefaultModuleVersionHandler {
    ...
    public VanityUrlModuleVersionHandler() {
        ...
        update154.addTask(new RemovePropertyTask("Remove wrong app activate config", "Remove recursive flag in activate action in app.", CONFIG, "/modules/magkit-vanity-url/apps/vanityUrl/subApps/browser/actions/activate", "recursive"));
        update154.addTask(new SetPropertyTask("Fix folder delete action config", CONFIG, "/modules/magkit-vanity-url/apps/vanityUrl/subApps/browser/actions/confirmDeleteFolder", "successActionName", "delete"));
        update154.addTask(new RemoveNodeTask("Remove wrong app action config", "Remove deleteFolder action in app.", CONFIG, "/modules/magkit-vanity-url/apps/vanityUrl/subApps/browser/actions/deleteFolder"));
        register(update154);

        DeltaBuilder update160 = DeltaBuilder.update("1.6.0", "Update to version 1.6.0");
        update160.addTask(new RemoveNodeTask("Remove jcr uri mapping config", "Remove virtual uri mapping in jcr. It's in yaml now.", CONFIG, "/modules/magkit-vanity-url/virtualURIMapping"));
        update160.addTask(new RemoveNodeTask("Remove jcr field types config", "Remove field types in jcr. No need for field types anymore.", CONFIG, "/modules/magkit-vanity-url/fieldTypes"));
        update160.addTask(new RemoveNodeTask("Remove jcr app config", "Remove app in jcr. It's in yaml now.", CONFIG, "/modules/magkit-vanity-url/apps"));
